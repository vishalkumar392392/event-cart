# Replaces the manual IAM policy `eks-secrets-manager-policy-2` from the doc.
...
resource "aws_iam_policy" "eks_secrets_manager" {
  name        = "eks-secrets-manager-policy-2"
...
#     --cluster eventcart-eks-01 --region us-east-2 \
#     --name eventcart-sa --namespace dev \
#     --attach-policy-arn arn:aws:iam::<acct>:policy/eks-secrets-manager-policy-2 \
#     --approve
#
...
resource "kubernetes_service_account" "eventcart_sa" {
  for_each = toset(var.irsa_namespaces)
  ...
  metadata {
    ...
    annotations = {
      "eks.amazonaws.com/role-arn" = module.irsa_eventcart_sa[each.value].iam_role_arn
    }
  }
